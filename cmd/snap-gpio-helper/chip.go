...
func (c *chipInfo) String() string {
	...
	s := fmt.Sprintf("(name: %s, ", string(c.name[:]))
	s += fmt.Sprintf("label: %s, ", string(c.label[:]))
	s += fmt.Sprintf("lines: %s)", strconv.FormatUint(uint64(c.lines), 10))
	return s
}

const _GPIO_GET_CHIPINFO_IOCTL uintptr = 0x8044b401

func getChipInfo(path string) (GPIOChardev, error) {
...
func findChips(filter func(chip GPIOChardev) bool) ([]GPIOChardev, error) {
	allPaths, err := filepath.Glob("/dev/gpiochip*")
	if err != nil {
		return nil, err
	}
